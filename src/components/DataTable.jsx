...
import DataCard from './DataCard'; // Import komponen kartu baru
...
const DataTable = ({ paginatedViewData, expandedGroups, onToggleGroup, currentPage, totalPages, setCurrentPage, kebunPerPage, setKebunPerPage, totalRows }) => {
  ...
  // Kita tidak butuh allHeaders flat lagi karena kita pakai grup berbasis Card
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow-sm border border-gray-200">
        <table className="min-w-full border-collapse">
          <thead className="bg-slate-100">
            <tr>
              <th rowSpan={2} className="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider border border-gray-300 w-16">No.</th>
              <th rowSpan={2} className="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider border border-gray-300 w-32">Kebun</th>
              
              {/* Header Utama Grup (Hari Ini, SD Hari Ini, dst) */}
              {groupNames.map(groupName => (
                <th key={groupName} className="px-4 py-3 text-center text-xs font-bold text-slate-800 uppercase tracking-wider border border-gray-300 min-w-[200px]">
                  {groupName}
                </th>
              ))}
            </tr>
            {/* Hapus sub-header row (Rkap, Prognosa, dll) karena sekarang masuk ke dalam Card */}
          </thead>
                ...
                <React.Fragment key={groupView.group.groupName}>
                  {isFirstOnPage && (
                    <tr>
                      <td colSpan={groupNames.length + 2} className="p-0">
                        <div
                          className="flex items-center space-x-3 px-6 py-3 bg-blue-50/50 cursor-pointer hover:bg-blue-100/50 transition-colors duration-200"
                  ...
                  {isExpanded && groupView.items.map((item, index) => (
                    <tr key={item.kebun + index} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                      {/* Kolom Nomor */}
                      <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-slate-900 border border-gray-200 text-center">
                        {index + 1}
                      </td>
                      
                      {/* Kolom Kebun Manual */}
                      <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-slate-900 border border-gray-200">
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                          {item.kebun}
                        </span>
                      </td>

                      {/* Loop Render DataCard untuk setiap Grup */}
                      {groupNames.map(groupName => {
                        const keys = HEADER_GROUPS[groupName];
                        return (
                          <td key={groupName} className="px-2 py-2 align-top border border-gray-200 bg-gray-50/50">
                            <DataCard 
                              title={groupName} 
                              data={item} 
                              keys={keys} 
